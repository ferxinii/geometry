...
//
// LOCAL MODIFICATION: stores two point ids plus a pointer to an external
// s_dynarray, instead of two direct explicitPoint3D references. P() / Q()
// resolve the ids against that dynarray on every call.
//
// explicitPoint2D, implicitPoint2D_SSI, implicitPoint3D_LPI and
// implicitPoint3D_TPI below are UNCHANGED: they still take and store direct
// explicitPoint3D references and require the referenced objects to have a
// permanent, stable address for the lifetime of the point.
class implicitPoint3D_LNC : public genericPoint {
	const s_dynarray *registry; // s_dynarray of explicitPoint3D; ids below index into it
	int ip_id, iq_id; // The two points, as ids into *registry
	const double t; // The parameter (0 = ip, 1 = iq)

public:
	implicitPoint3D_LNC(const s_dynarray *_registry, int _p_id, int _q_id,
		const double _t);

	const explicitPoint3D& P() const { return *(const explicitPoint3D*)dynarray_get_ptr_c(registry, (size_t)ip_id); }
	const explicitPoint3D& Q() const { return *(const explicitPoint3D*)dynarray_get_ptr_c(registry, (size_t)iq_id); }
